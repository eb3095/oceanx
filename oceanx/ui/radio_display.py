"""Marine VHF radio monitor dashboard."""
...
from oceanx.config import MAX_RADIO_TRANSCRIPTS, WAVEFORM_HEIGHT, RadioConfig
from oceanx.dsp.waveform import WaveformView
from oceanx.radio.voice_monitor import TranscriptLine, VoiceMonitor
from oceanx.ui.bars import FullWidthBar
from oceanx.ui.display import _dim_text
...
class RadioDisplay:
    ...
    def render(self) -> Group:
        ...
        return Group(
            FullWidthBar(f" {__app_name__} — Marine VHF Radio "),
    ...
    def _transcript_panel(self, channel) -> Panel:
        ...
        if not lines:
            body = _dim_text(
                f"Listening on {channel.freq_mhz:.3f} MHz — {channel.name}. Transmissions appear here."
            )
        else:
        ...
        return Panel(
            body,
            title=f"[bold]Live Transcript[/bold] — {channel.name} ({channel.freq_mhz:.3f} MHz) · last {MAX_RADIO_TRANSCRIPTS}",
            border_style="green",
            padding=(0, 1),
            style="none",
        )
